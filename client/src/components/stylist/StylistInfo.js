...
const StylistInfo = () => {
  ...
  useEffect(() => {
    const fetchStylist = async () => {
      await axios
        .get(`${process.env.REACT_APP_BACKEND}/api/stylists/${stylistId.id}`)
        .then((res) => {
                ...
                <div className="col-9">
                  ...
                  <ul className="list-group list-group-flush">
                    <li className="list-group-item">
                      Phone Number:{" "}
                      {stylist.number === "" ? "None" : stylist.number}
                    </li>
                    <li className="list-group-item">
                      Email: {stylist.email === "" ? "None" : stylist.email}
                    </li>
                    <li className="list-group-item">
                      Address:{" "}
                      {stylist.address === "" ? "None" : stylist.address}
                    </li>
                  </ul>
                </div>
              </div>
