...
const Services = ({ services, stylistId }) => {
  ...
  const URL = `${process.env.REACT_APP_BACKEND}/api/stylists/appointments/`;
  useEffect(() => {
    const getAppointments = async () => {
      await axios
        ...
        .then((res) => {
          ...
          const availableAppointments = returnedAppointments.filter(
            (appointment) =>
              appointment.user === null && new Date(appointment.endDate) > now
          );
          availableAppointments.sort(
            (app1, app2) => new Date(app1.endDate) - new Date(app2.endDate)
          );
